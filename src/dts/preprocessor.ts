...
function evaluate(text: string, loc: vscode.Location, defines: Defines): any {
  ...
  try {
    const result = evaluateExpr(text, loc.range.start);
    return result;
  } catch (e) {
  }
    ...
    try {
      ...
      if (directive) {
        ...
        if (directive[1] === 'if') {
          ...
          value = value.replace(new RegExp(`defined\\((.*?)\\)`, 'g'), (_t, define) => {
        ...
        if (directive[1] === 'elif') {
          ...
          condition = condition.replace(new RegExp(`defined\\((.*?)\\)`, 'g'), (_t, define) => {
            return result.defines[define]?.isDefined ? '1' : '0';
          });
