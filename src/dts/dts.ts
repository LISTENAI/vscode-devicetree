...
export class StringValue extends PropertyValue {
  constructor(public val: string, loc: vscode.Location) {
    super(val, loc);
  }

  static match(state: ParserState): StringValue | undefined {
...
export class BoolValue extends PropertyValue {
  ...
  toString(_raw = false): string {
    return this.val.toString();
  }
}

export class IntValue extends PropertyValue {
  protected constructor(public raw: string, public val: number, loc: vscode.Location) {
    super(val, loc);
  }

  static match(state: ParserState): IntValue | undefined {
...
export class Expression extends IntValue {
  static match(state: ParserState): Expression | undefined {
...
export class ArrayValue extends PropertyValue {
  private constructor(public val: (PHandle | IntValue | Expression)[], loc: vscode.Location) {
    super(val, loc);
  }

  static match(state: ParserState): ArrayValue | undefined {
    const start = state.freeze();
...
export class BytestringValue extends PropertyValue {
  private constructor(public val: number[], loc: vscode.Location) {
    super(val, loc);
  }
  ...
  static match(state: ParserState): BytestringValue | undefined {
    if (!state.match(/^\[/)) {
      return;
    }
...
export class PHandle extends PropertyValue {
  private constructor(public val: string, loc: vscode.Location, public kind: 'ref' | 'pathRef' | 'string' | 'invalid') {
    super(val, loc);
  }
  ...
  static match(state: ParserState): PHandle | undefined {
...
function parsePropValue(state: ParserState) {
  ...
  let missingComma: vscode.Location | null = null;
  ...
  while (state.skipWhitespace()) {
    ...
    let match: PropertyValue | undefined;
...
export class Property {
  ...
  get valueLoc() {
    const range = this.value.reduce((union, v) => {
      ...
      return v.loc.range;
    }, <vscode.Range | undefined>undefined);
  ...
  get arrays() {
    if (this.value.every(v => v instanceof ArrayValue && v.val.every(v => v instanceof IntValue))) {
      return this.value.map(v => v.val.map((v: ArrayValue | IntValue) => v.val) as number[]);
    }
  }
  ...
  type(): string {
    ...
    if (this.value.every(v => v instanceof ArrayValue)) {
      ...
      if (this.value.every(v => v.val.every((e: any) => e instanceof PHandle))) {
        return 'phandles';
      }

      if (this.value.every(v => v.val.every((e: any) => e instanceof IntValue))) {
        return 'array';
      }
...
export class Node {
  ...
  remap(name: string, entry: PHandleEntry): PHandleEntry | undefined {
  ...
  refCellNames(prop: string): string[] | undefined {
...
export class DTSFile {
  ...
  getEntryAt(pos: vscode.Position, uri: vscode.Uri): NodeEntry | undefined {
  ...
  getPropertyAt(pos: vscode.Position, uri: vscode.Uri): Property | undefined {
...
export class DTSCtx {
  ...
  node(name: string, parent?: Node): Node | undefined {
    if (name.startsWith('&{')) {
      ...
      return Object.values(this.nodes).find(n => n.hasLabel(ref));
    }
  ...
  getPHandleNode(handle: number | string): Node | undefined {
...
export class Parser {
  ...
  set currCtx(ctx: DTSCtx | undefined) {
    this._currCtx = ctx;
  }

  get currCtx(): DTSCtx | undefined {
    if (this.inDTS) {
  ...
  async addContext(board?: vscode.Uri | zephyr.Board, overlays = <vscode.Uri[]>[], name?: string): Promise<DTSCtx | undefined> {
    ...
    ctx.parsing = false;
    ctx.overlays = (await Promise.all(overlays.map(uri => vscode.workspace.openTextDocument(uri).then(doc => this.parse(ctx, doc), () => undefined)))).filter(d => d) as DTSFile[];
